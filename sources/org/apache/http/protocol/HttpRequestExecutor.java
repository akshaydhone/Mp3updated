...
public class HttpRequestExecutor {
    ...
    protected boolean canResponseHaveBody(HttpRequest request, HttpResponse response) {
        if (io.fabric.sdk.android.services.network.HttpRequest.METHOD_HEAD.equalsIgnoreCase(request.getRequestLine().getMethod())) {
            return false;
        }
        int status = response.getStatusLine().getStatusCode();
        if (status < 200 || status == HttpStatus.SC_NO_CONTENT || status == 304 || status == HttpStatus.SC_RESET_CONTENT) {
            return false;
        }
        return true;
    }
