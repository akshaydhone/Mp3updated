...
class FabricKitsFinder implements Callable<Map<String, KitInfo>> {
    ...
    public Map<String, KitInfo> call() throws Exception {
        ...
        Fabric.getLogger().mo1340v(Fabric.TAG, "finish scanning in " + (SystemClock.elapsedRealtime() - startScan));
        return kitInfos;
    }
    ...
    private Map<String, KitInfo> findImplicitKits() {
        ...
        try {
            ...
            KitInfo admobKitInfo = new KitInfo("com.google.firebase.firebase-ads", "0.0.0", MIME.ENC_BINARY);
            implicitKits.put(admobKitInfo.getIdentifier(), admobKitInfo);
            Fabric.getLogger().mo1340v(Fabric.TAG, "Found kit: com.google.firebase.firebase-ads");
        } catch (Exception e) {
        }
        return implicitKits;
    }
        ...
        while (entries.hasMoreElements()) {
            ...
            if (entry.getName().startsWith(FABRIC_DIR) && entry.getName().length() > FABRIC_DIR.length()) {
                ...
                if (kitInfo != null) {
                    ...
                    Fabric.getLogger().mo1340v(Fabric.TAG, String.format("Found kit:[%s] version:[%s]", new Object[]{kitInfo.getIdentifier(), kitInfo.getVersion()}));
                }
            }
        }
        if (apkFile != null) {
    ...
    private KitInfo loadKitInfo(ZipEntry fabricFile, ZipFile apk) {
        ...
        try {
            ...
            kitInfo.mo1332e(Fabric.TAG, "Error when parsing fabric properties " + fabricFile.getName(), ie);
            return null;
        } finally {
