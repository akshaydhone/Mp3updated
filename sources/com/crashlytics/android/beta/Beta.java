...
public class Beta extends Kit<Boolean> implements DeviceIdentifierProvider {
    ...
    protected Boolean doInBackground() {
        Fabric.getLogger().mo1329d(TAG, "Beta kit initializing...");
        Context context = getContext();
        IdManager idManager = getIdManager();
        if (TextUtils.isEmpty(getBetaDeviceToken(context, idManager.getInstallerPackageName()))) {
            Fabric.getLogger().mo1329d(TAG, "A Beta device token was not found for this app");
            return Boolean.valueOf(false);
        }
        Fabric.getLogger().mo1329d(TAG, "Beta device token is present, checking for app updates.");
    ...
    private String getBetaDeviceToken(Context context, String installerPackageName) {
        ...
        try {
            ...
            Fabric.getLogger().mo1332e(TAG, "Failed to load the Beta device token", e);
        }
        Fabric.getLogger().mo1329d(TAG, "Beta device token present: " + (!TextUtils.isEmpty(token)));
        return token;
    }
    ...
    private BuildProperties loadBuildProperties(Context context) {
        ...
        try {
            ...
            if (buildPropsStream != null) {
                ...
                Fabric.getLogger().mo1329d(TAG, buildProps.packageName + " build properties: " + buildProps.versionName + " (" + buildProps.versionCode + ") - " + buildProps.buildId);
            }
            if (buildPropsStream != null) {
                try {
                    buildPropsStream.close();
                } catch (IOException e) {
                    Fabric.getLogger().mo1332e(TAG, "Error closing Beta build properties asset", e);
                }
            }
        } catch (Exception e2) {
            Fabric.getLogger().mo1332e(TAG, "Error reading Beta build properties", e2);
            if (buildPropsStream != null) {
                try {
                    buildPropsStream.close();
                } catch (IOException e3) {
                    Fabric.getLogger().mo1332e(TAG, "Error closing Beta build properties asset", e3);
                }
            }
        } catch (Throwable th) {
            if (buildPropsStream != null) {
                try {
                    ...
                    Fabric.getLogger().mo1332e(TAG, "Error closing Beta build properties asset", e32);
                }
            }
        }
        return buildProps;
