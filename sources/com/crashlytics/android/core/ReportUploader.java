...
class ReportUploader {
    ...
    private class Worker extends BackgroundPriorityRunnable {
        ...
        public void onRun() {
            try {
                ...
                Fabric.getLogger().mo1332e(CrashlyticsCore.TAG, "An unexpected error occurred while attempting to upload crash reports.", e);
            }
            ReportUploader.this.uploadThread = null;
        }

        private void attemptUploadWithRetry() {
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Starting report processing in " + this.delay + " second(s)...");
            if (this.delay > 0.0f) {
                try {
            ...
            if (!ReportUploader.this.handlingExceptionCheck.isHandlingException()) {
                if (reports.isEmpty() || this.sendCheck.canSendReports()) {
                    ...
                    while (!reports.isEmpty() && !ReportUploader.this.handlingExceptionCheck.isHandlingException()) {
                        Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Attempting to send " + reports.size() + " report(s)");
                        ...
                        if (!reports.isEmpty()) {
                            ...
                            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Report submisson: scheduling delayed retry in " + interval + " seconds");
                            try {
                ...
                Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "User declined to send. Removing " + reports.size() + " Report(s).");
    ...
    public synchronized void uploadReports(float delay, SendCheck sendCheck) {
        if (this.uploadThread != null) {
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Report upload has already been started.");
        } else {
    ...
    boolean forceUpload(Report report) {
        ...
        synchronized (this.fileAccessLock) {
            try {
                ...
                Fabric.getLogger().mo1334i(CrashlyticsCore.TAG, "Crashlytics report upload " + (sent ? "complete: " : "FAILED: ") + report.getIdentifier());
                if (sent) {
                    report.remove();
                    removed = true;
                }
            } catch (Exception e) {
                Fabric.getLogger().mo1332e(CrashlyticsCore.TAG, "Error occurred sending report " + report, e);
            }
        }
        return removed;
    }
    ...
    List<Report> findReports() {
        Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Checking for crash reports...");
        synchronized (this.fileAccessLock) {
        ...
        if (clsFiles != null) {
            for (File file : clsFiles) {
                Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Found crash report " + file.getPath());
        ...
        for (String key : invalidSessionFiles.keySet()) {
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Found invalid session: " + key);
        ...
        if (reports.isEmpty()) {
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "No reports found.");
        }
        return reports;
    }
}
