...
class CrashlyticsBackgroundWorker {
    ...
    <T> T submitAndWait(Callable<T> callable) {
        try {
            ...
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Executor is shut down because we're handling a fatal crash.");
            return null;
        } catch (Exception e2) {
            Fabric.getLogger().mo1332e(CrashlyticsCore.TAG, "Failed to execute task.", e2);
            return null;
        }
    }
            ...
            return this.executorService.submit(new Runnable() {
                public void run() {
                    try {
                        ...
                        Fabric.getLogger().mo1332e(CrashlyticsCore.TAG, "Failed to execute task.", e);
                    }
                }
            });
        } catch (RejectedExecutionException e) {
