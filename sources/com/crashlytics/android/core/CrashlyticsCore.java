...
public class CrashlyticsCore extends Kit<Void> {
    ...
    class C02512 implements Callable<Void> {
        C02512() {
        }

        public Void call() throws Exception {
            CrashlyticsCore.this.initializationMarker.create();
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Initialization marker file created.");
            return null;
        }
    }

    /* renamed from: com.crashlytics.android.core.CrashlyticsCore$3 */
    class C02523 implements Callable<Boolean> {
        C02523() {
        }
        ...
        public Boolean call() throws Exception {
            try {
                ...
                Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Initialization marker file removed: " + removed);
                return Boolean.valueOf(removed);
            } catch (Exception e) {
                Fabric.getLogger().mo1332e(CrashlyticsCore.TAG, "Problem encountered deleting Crashlytics initialization marker.", e);
    ...
    class C02534 implements Callable<Boolean> {
        C02534() {
        }
    ...
    private static final class CrashMarkerCheck implements Callable<Boolean> {
        ...
        public Boolean call() throws Exception {
            ...
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Found previous crash marker.");
    ...
    class C07141 extends PriorityCallable<Void> {
        C07141() {
        }
    ...
    boolean onPreExecute(Context context) {
        ...
        if (isBuildIdValid(buildId, CommonUtils.getBooleanResourceValue(context, CRASHLYTICS_REQUIRE_BUILD_ID, true))) {
            try {
                Fabric.getLogger().mo1334i(TAG, "Initializing Crashlytics " + getVersion());
                ...
                Fabric.getLogger().mo1329d(TAG, "Installer package name is: " + appData.installerPackageName);
                ...
                if (initializeSynchronously && CommonUtils.canTryConnection(context)) {
                    Fabric.getLogger().mo1329d(TAG, "Crashlytics did not finish previous background initialization. Initializing synchronously.");
                    finishInitSynchronously();
                    return false;
                }
                Fabric.getLogger().mo1329d(TAG, "Exception handling initialization successful");
                return true;
            } catch (Exception e) {
                Fabric.getLogger().mo1332e(TAG, "Crashlytics was not started due to an exception during initialization", e);
    ...
    protected Void doInBackground() {
        ...
        try {
            ...
            if (settingsData == null) {
                Fabric.getLogger().mo1342w(TAG, "Received null settings, skipping report submission!");
            } else if (settingsData.featuresData.collectReports) {
                if (!this.controller.finalizeSessions(settingsData.sessionData)) {
                    Fabric.getLogger().mo1329d(TAG, "Could not finalize previous sessions.");
                }
                this.controller.submitAllReports(this.delay, settingsData);
                markInitializationComplete();
            } else {
                Fabric.getLogger().mo1329d(TAG, "Collection of crash reports disabled in Crashlytics settings.");
                markInitializationComplete();
            }
        } catch (Exception e) {
            Fabric.getLogger().mo1332e(TAG, "Crashlytics encountered a problem during asynchronous initialization.", e);
        } finally {
            markInitializationComplete();
        }
        return null;
    ...
    public void setString(String key, String value) {
        ...
        if (key == null) {
            ...
            if (context == null || !CommonUtils.isAppDebuggable(context)) {
                Fabric.getLogger().mo1332e(TAG, "Attempting to set custom attribute with null key, ignoring.", null);
                return;
            }
        ...
        Fabric.getLogger().mo1329d(TAG, "Exceeded maximum number of custom attributes (64)");
    }
    ...
    public boolean verifyPinning(URL url) {
        try {
            ...
            Fabric.getLogger().mo1332e(TAG, "Could not verify SSL pinning", e);
            return false;
        }
    }
    ...
    public synchronized void setListener(CrashlyticsListener listener) {
        Fabric.getLogger().mo1342w(TAG, "Use of setListener is deprecated.");
        if (listener == null) {
            throw new IllegalArgumentException("listener must not be null.");
        }
        this.listener = listener;
    ...
    private void finishInitSynchronously() {
        PriorityCallable<Void> callable = new C07141();
        for (Task task : getDependencies()) {
            callable.addDependency(task);
        }
        Future<Void> future = getFabric().getExecutorService().submit(callable);
        Fabric.getLogger().mo1329d(TAG, "Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            future.get(4, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Fabric.getLogger().mo1332e(TAG, "Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Fabric.getLogger().mo1332e(TAG, "Problem encountered during Crashlytics initialization.", e2);
        } catch (TimeoutException e3) {
            Fabric.getLogger().mo1332e(TAG, "Crashlytics timed out during initialization.", e3);
        }
    }

    void markInitializationStarted() {
        this.backgroundWorker.submitAndWait(new C02512());
    }

    void markInitializationComplete() {
        this.backgroundWorker.submit(new C02523());
    }

    boolean didPreviousInitializationFail() {
        return ((Boolean) this.backgroundWorker.submitAndWait(new C02534())).booleanValue();
    }
    ...
    private void checkForPreviousCrash() {
        if (Boolean.TRUE.equals((Boolean) this.backgroundWorker.submitAndWait(new CrashMarkerCheck(this.crashMarker)))) {
            try {
                ...
                Fabric.getLogger().mo1332e(TAG, "Exception thrown by CrashlyticsListener while notifying of previous crash.", e);
            }
        }
    }
    ...
    private static boolean ensureFabricWithCalled(String msg) {
        ...
        Fabric.getLogger().mo1332e(TAG, "Crashlytics must be initialized by calling Fabric.with(Context) " + msg, null);
        return false;
    }
    ...
    static boolean isBuildIdValid(String buildId, boolean requiresBuildId) {
        if (!requiresBuildId) {
            Fabric.getLogger().mo1329d(TAG, "Configured not to require a build ID.");
            return true;
