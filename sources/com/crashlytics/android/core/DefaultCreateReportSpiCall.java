...
class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest requestData) {
        ...
        Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Sending report to: " + getUrl());
        int statusCode = httpRequest.code();
        Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Create report request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
        Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Result was: " + statusCode);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest request, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return request.part(FILE_PARAM, report.getFileName(), "application/octet-stream", report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Fabric.getLogger().mo1329d(CrashlyticsCore.TAG, "Adding file " + file.getName() + " to report " + report.getIdentifier());
            request.part(MULTI_FILE_PARAM + i + "]", file.getName(), "application/octet-stream", file);
            i++;
        }
        return request;
    }
