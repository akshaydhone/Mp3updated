...
class EnabledSessionAnalyticsManagerStrategy implements SessionAnalyticsManagerStrategy {
    ...
    public void setAnalyticsSettingsData(AnalyticsSettingsData analyticsSettingsData, String protocolAndHostOverride) {
        ...
        Fabric.getLogger().mo1329d(Answers.TAG, "Custom event tracking " + (this.customEventsEnabled ? "enabled" : "disabled"));
        this.predefinedEventsEnabled = analyticsSettingsData.trackPredefinedEvents;
        Fabric.getLogger().mo1329d(Answers.TAG, "Predefined event tracking " + (this.predefinedEventsEnabled ? "enabled" : "disabled"));
        if (analyticsSettingsData.samplingRate > 1) {
            Fabric.getLogger().mo1329d(Answers.TAG, "Event sampling enabled");
    ...
    public void processEvent(Builder builder) {
        ...
        if (!this.customEventsEnabled && Type.CUSTOM.equals(event.type)) {
            Fabric.getLogger().mo1329d(Answers.TAG, "Custom events tracking disabled - skipping event: " + event);
        } else if (!this.predefinedEventsEnabled && Type.PREDEFINED.equals(event.type)) {
            Fabric.getLogger().mo1329d(Answers.TAG, "Predefined events tracking disabled - skipping event: " + event);
        } else if (this.eventFilter.skipEvent(event)) {
            Fabric.getLogger().mo1329d(Answers.TAG, "Skipping filtered event: " + event);
        } else {
            try {
                this.filesManager.writeEvent(event);
            } catch (IOException e) {
                Fabric.getLogger().mo1332e(Answers.TAG, "Failed to write event: " + event, e);
            }
