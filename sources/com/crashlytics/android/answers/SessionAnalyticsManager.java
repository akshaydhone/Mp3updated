...
class SessionAnalyticsManager implements Listener {
    ...
    public void onCustom(CustomEvent event) {
        Fabric.getLogger().mo1329d(Answers.TAG, "Logged custom event: " + event);
        this.eventsHandler.processEventAsync(SessionEvent.customEventBuilder(event));
    }

    public void onPredefined(PredefinedEvent event) {
        Fabric.getLogger().mo1329d(Answers.TAG, "Logged predefined event: " + event);
    ...
    public void onCrash(String sessionId, String exceptionName) {
        ...
        Fabric.getLogger().mo1329d(Answers.TAG, "Logged crash");
    ...
    public void onInstall(long installedAt) {
        Fabric.getLogger().mo1329d(Answers.TAG, "Logged install");
    ...
    public void onLifecycle(Activity activity, Type type) {
        Fabric.getLogger().mo1329d(Answers.TAG, "Logged lifecycle event: " + type.name());
        this.eventsHandler.processEventAsync(SessionEvent.lifecycleEventBuilder(type, activity));
    }

    public void onBackground() {
        Fabric.getLogger().mo1329d(Answers.TAG, "Flush events when app is backgrounded");
