...
public class Answers extends Kit<Boolean> {
    ...
    protected boolean onPreExecute() {
        try {
            ...
            Fabric.getLogger().mo1332e(TAG, "Error retrieving app properties", e);
            return false;
        }
    }
    ...
    protected Boolean doInBackground() {
        try {
            ...
            if (settingsData == null) {
                Fabric.getLogger().mo1331e(TAG, "Failed to retrieve settings");
                return Boolean.valueOf(false);
            } else if (settingsData.featuresData.collectAnalytics) {
                Fabric.getLogger().mo1329d(TAG, "Analytics collection enabled");
                this.analyticsManager.setAnalyticsSettingsData(settingsData.analyticsSettingsData, getOverridenSpiEndpoint());
                return Boolean.valueOf(true);
            } else {
                Fabric.getLogger().mo1329d(TAG, "Analytics collection disabled");
                this.analyticsManager.disable();
                return Boolean.valueOf(false);
            }
        } catch (Exception e) {
            Fabric.getLogger().mo1332e(TAG, "Error dealing with settings", e);
            return Boolean.valueOf(false);
        }
    }
