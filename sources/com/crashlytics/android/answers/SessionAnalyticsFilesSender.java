...
class SessionAnalyticsFilesSender extends AbstractSpiCall implements FilesSender {
    ...
    public boolean send(List<File> files) {
        ...
        Fabric.getLogger().mo1329d(Answers.TAG, "Sending " + files.size() + " analytics files to " + getUrl());
        int statusCode = httpRequest.code();
        Fabric.getLogger().mo1329d(Answers.TAG, "Response code for analytics file send is " + statusCode);
