...
public abstract class TextBoxBase extends AndroidViewComponent implements OnFocusChangeListener {
    ...
    public TextBoxBase(ComponentContainer container, EditText textview) {
        ...
        UnderlineColor(Component.MATERIALCOLOR_LIGHT_GREEN);
        ...
        Enabled(true);
        this.fontTypeface = 0;
        TextViewUtil.setContext(container.$context());
    ...
    @SimpleProperty(category = PropertyCategory.APPEARANCE, description = "The underline color of the input box. You can choose a color by name in the Designer or in the Blocks Editor.  The default background color is 'default' (shaded 3-D look).")
    public int UnderlineColor() {
        return this.underlineColor;
    }

    @DesignerProperty(defaultValue = "&HFF82CCC4", editorType = "color")
    @SimpleProperty
    public void UnderlineColor(int argb) {
        this.underlineColor = argb;
        TextViewUtil.setCursorColor(this.view, this.underlineColor);
        if (this.backgroundColor == 0) {
            if (SdkLevel.getLevel() >= 21) {
                LollipopUtil.setTextBoxBackgroundTintList(this.view, this.underlineColor);
            } else {
                this.view.getBackground().setColorFilter(this.underlineColor, Mode.SRC_IN);
            }
        }
    }
    ...
    public void Hint(String hint) {
        this.hint = hint;
        this.view.setHintTextColor(Component.COLOR_GRAY);
    ...
    public void FontTypefaceImport(String path) {
        ...
        if (path != "") {
            TextViewUtil.setCustomFontTypeface(this.container.$form(), this.view, path, this.bold, this.italic);
        } else {
