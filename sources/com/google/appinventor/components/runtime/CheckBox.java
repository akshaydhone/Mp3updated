...
@SimpleObject
@DesignerComponent(category = ComponentCategory.USERINTERFACE, description = "Checkbox that raises an event when the user clicks on it. There are many properties affecting its appearance that can be set in the Designer or Blocks Editor.", docUri = "user-interface/listpicker", version = 2)
public final class CheckBox extends AndroidViewComponent implements OnCheckedChangeListener, OnFocusChangeListener {
    ...
    public CheckBox(ComponentContainer container) {
        ...
        OutlineColor(Component.MATERIALCOLOR_LIGHT_GREEN);
        container.$add(this);
    ...
    @SimpleProperty
    public void OutlineColor(int argb) {
        this.outlineColor = argb;
        if (SdkLevel.getLevel() >= 21) {
            LollipopUtil.setCheckBoxButtonTintList(this.view, this.outlineColor);
        }
    }
