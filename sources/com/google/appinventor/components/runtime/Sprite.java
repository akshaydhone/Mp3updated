...
public abstract class Sprite extends VisibleComponent implements AlarmHandler, OnDestroyListener, Deleteable {
    ...
    public void PointTowards(Sprite target) {
        Heading(-Math.toDegrees(Math.atan2((target.m46Y() - m46Y()) + ((double) ((target.Height() - Height()) / 2)), (target.m44X() - m44X()) + ((double) ((target.Width() - Width()) / 2)))));
    }
    ...
    public void PointInDirection(double x, double y) {
        Heading(-Math.toDegrees(Math.atan2((y - m46Y()) - ((double) (Height() / 2)), (x - m44X()) - ((double) (Width() / 2)))));
    }
    ...
    public BoundingBox getBoundingBox(int border) {
        return new BoundingBox(m44X() - ((double) border), m46Y() - ((double) border), ((m44X() + ((double) Width())) - DEFAULT_Z) + ((double) border), ((m46Y() + ((double) Height())) - DEFAULT_Z) + ((double) border));
    }
