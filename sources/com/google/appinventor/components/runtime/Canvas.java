...
public final class Canvas extends AndroidViewComponent implements ComponentContainer {
    ...
    private final MultiTouchDetector multiTouchDetector;
        ...
        protected void onSizeChanged(int w, int h, int oldW, int oldH) {
            ...
            if (w != oldBitmapWidth || h != oldBitmapHeight) {
                ...
                try {
                    ...
                    Log.e("Canvas", "Bad values to createScaledBimap w = " + w + ", h = " + h);
                }
            ...
            switch (event.getAction()) {
                ...
                case 2:
                    if (this.startX == -1.0f || this.startY == -1.0f || this.lastX == -1.0f || this.lastY == -1.0f) {
                        Log.w("Canvas", "In Canvas.MotionEventParser.parse(), an ACTION_MOVE was passed without a preceding ACTION_DOWN: " + event);
                    }
    ...
    public void setFontTypeface(int typeface) {
        ...
        switch (typeface) {
            ...
            case 3:
                ...
                break;
            case 4:
                tf = Typeface.createFromAsset(this.context.getAssets(), "Thunkable-Roboto-Regular.ttf");
                break;
            case 5:
                tf = Typeface.createFromAsset(this.context.getAssets(), "Thunkable-Roboto-Thin.ttf");
                break;
            default:
        ...
        this.paint.setTypeface(Typeface.create(tf, style));
    }
    ...
    public Canvas(ComponentContainer container) {
        ...
        this.multiTouchDetector = new MultiTouchDetector(this);
    }
    ...
    void addSprite(Sprite sprite) {
        for (int i = 0; i < this.sprites.size(); i++) {
            if (((Sprite) this.sprites.get(i)).m48Z() > sprite.m48Z()) {
    ...
    public void FontTypefaceImport(String path) {
        Context ct = this.context;
        if (path == null) {
            path = "";
        }
        if (path != "") {
            Typeface tf;
            String typefacePath = MediaUtil.getAssetFilePath(this.container.$form(), path);
        ...
        Log.e("Label", "Unable to load " + path);
    }
    ...
    @SimpleEvent(description = "This event is invoked when two-finger pinches. scaleFactor is the ratio of the average distance between two-fingers from current and previous scale event. When zooming out, it would be greater than one, and vice versa.")
    public void Scale(double scaleFactor) {
        EventDispatcher.dispatchEvent(this, "Scale", Double.valueOf(scaleFactor));
    }
