...
public class MemoryLeakUtil {
    ...
    public static String trackObject(String tag, Object object) {
        String key;
        if (tag == null) {
            key = prefixGenerator.incrementAndGet() + EventsFilesManager.ROLL_OVER_FILE_NAME_SEPARATOR;
        } else {
            key = prefixGenerator.incrementAndGet() + EventsFilesManager.ROLL_OVER_FILE_NAME_SEPARATOR + tag;
        }
    ...
    public static boolean isTrackedObjectCollected(String key, boolean stopTrackingIfCollected) {
        ...
        if (ref != null) {
            Object o = ref.get();
            Log.i(LOG_TAG, "Object with tag " + key.substring(key.indexOf(EventsFilesManager.ROLL_OVER_FILE_NAME_SEPARATOR) + 1) + " has " + (o != null ? "not " : "") + "been garbage collected.");
    ...
    public static void checkAllTrackedObjects(boolean verbose, boolean stopTrackingCollectedObjects) {
        ...
        while (it.hasNext()) {
            ...
            if (verbose) {
                String str;
                String tag = key.substring(key.indexOf(EventsFilesManager.ROLL_OVER_FILE_NAME_SEPARATOR) + 1);
                String str2 = LOG_TAG;
